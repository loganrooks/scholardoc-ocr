...
must_haves:
  ...
  artifacts:
    - path: "src/scholardoc_ocr/quality.py"
      ...
      exports: ["QualityAnalyzer", "QualityResult"]
...
1. Import SignalResult from types.py: `from scholardoc_ocr.types import SignalResult`. Import DictionarySignal from dictionary.py. Import ConfidenceSignal from confidence.py.
...
Note on QUAL-03 (per-page quality breakdown): QualityResult now includes signal_scores and signal_details fields, providing the data layer for per-page breakdown. Wiring this data into PageResult/FileResult in the pipeline is deferred to Phase 4 (pipeline orchestration), as Phase 2 scope is limited to quality measurement enhancements only.
  </action>
  <verify>
...
Target: 15-20 test functions covering above categories. Prioritize categories 1-5 and 8 (core behavior and backward compat). Categories 6, 7, 9 can have 1-2 tests each. If context pressure grows, reduce edge case coverage rather than cutting core tests.
  </action>
  <verify>

...
<task type="auto">
  ...
  <files>src/scholardoc_ocr/dictionary.py, src/scholardoc_ocr/data/wordlist.txt</files>
  <action>
...
Define `SignalResult` as a dataclass in dictionary.py (will be shared later):
```python
@dataclass
class SignalResult:
    name: str
...
<verification>
- `python -c "from scholardoc_ocr.dictionary import DictionarySignal, SignalResult"` imports succeed

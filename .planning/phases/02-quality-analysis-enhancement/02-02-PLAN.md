...
2. Import `SignalResult` from dictionary.py (or define locally if dictionary.py not yet available — use try/except import).
...
`python -c "from scholardoc_ocr.confidence import ConfidenceSignal; s = ConfidenceSignal(); r = s.score_from_data([]); print(r.score)"` returns 0.5 (empty data).
  ...
  <done>Confidence module extracts word-level confidence from PDF pages via pytesseract, filters non-text entries, produces weighted 0-1 score. pytesseract declared as dependency.</done>
</task>

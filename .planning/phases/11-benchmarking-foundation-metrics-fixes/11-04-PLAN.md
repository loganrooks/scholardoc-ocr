...
1. Import timing utilities at top of file:
   ```python
...
1. Import compute_engine_from_pages:
   ```python
   from .types import BatchResult, FileResult, OCREngine, PageResult, PageStatus, compute_engine_from_pages
   ```

2. Add quality re-evaluation after Surya enhancement. Inside the `for file_result in flagged_results:` loop, after updating PageResult entries (~line 438-443):
   ...
   # Get output PDF path for text extraction
   pdf_path = config.output_dir / "final" / f"{input_path.stem}.pdf"
   ...
   if text_path.exists():
       ...
       for page in file_result.pages:
           if page.page_number in bad_indices:
               # Preserve pre-Surya quality score
               page_pre_surya_score = page.quality_score
...
- Manual inspection confirms:
  ...
  - Quality re-evaluated for Surya-enhanced pages
</verification>

...
2. Note on strict_gpu:
   - strict_gpu is NOT enforced at model load time - model loading should always succeed
   - strict_gpu is enforced at INFERENCE time in Plan 12-05's convert_pdf_with_fallback()
   - For now, just add the field to config for propagation to inference
  </action>
  <verify>
...
3. Test CLI parses --strict-gpu flag (use ArgumentParser directly):
   ```python
   def test_cli_strict_gpu_flag():
       """Test that --strict-gpu flag is recognized by ArgumentParser."""
       import argparse
       from scholardoc_ocr.cli import create_parser  # Assumes parser creation is factored out
       # If create_parser doesn't exist, test via subprocess instead:
       import subprocess
   ...
   Alternative implementation if create_parser exists:
   ```python
   def test_cli_strict_gpu_flag():
       from scholardoc_ocr.cli import create_parser
       parser = create_parser()
       args = parser.parse_args(["--strict-gpu", "."])
       assert args.strict_gpu is True
   ```
  </action>
  <verify>

...
2. **on_phase(event: PhaseEvent):**
   ...
   - On `status == "completed"`: Guard against double-stop: only call `progress.stop()` if `_progress is not None`, then set `_progress = None` and `_task_id = None`.
...
5. **Error safety:** Wrap all Rich rendering calls (progress.update, progress.stop, console.print) in try/except that catches `Exception` and falls back to plain `print()`. This prevents Rich rendering failures (e.g., pipe to file, broken terminal) from crashing the pipeline.
...
<task type="auto">
  <name>Task 2: Add new flags and integrate RichCallback</name>
  <files>src/scholardoc_ocr/cli.py</files>
  <action>
Rewrite the `main()` function's argument parsing and pipeline invocation:
...
2. **--files flag path resolution:** When `-f/--files` is provided, resolve each filename relative to `input_dir` (i.e., `input_dir / filename`). If a path is absolute, use it as-is. Validate that each resolved path exists and is a `.pdf` file; print a warning and skip non-existent files.

3. **Console and callback setup:**
   - Create `Console(no_color=args.no_color or os.environ.get("NO_COLOR") is not None)`.
...
Verify cli.py imports ONLY from pipeline, callbacks, types (plus stdlib and rich).
  ...
  <done>CLI has all new flags (--output-dir, --language, --no-color, --version) plus all existing flags. RichCallback is wired to run_pipeline. --files resolves paths relative to input_dir with existence validation.</done>
</task>

<task type="auto">
  <name>Task 3: Rich summary table and error handling</name>
  <files>src/scholardoc_ocr/cli.py</files>
  <action>
1. **Rich summary table (replace `_print_summary()`):**
...
2. **Error handling:** Wrap `run_pipeline()` in try/except. On unexpected exceptions, print clean error message (not traceback) unless --debug is set. On KeyboardInterrupt, print "Interrupted" and exit 130.

3. **Exit codes:** 0 if no errors, 1 if any errors. Same as current behavior.
  </action>
  <verify>
Run `ruff check src/scholardoc_ocr/cli.py` passes.
Inspect cli.py to confirm Rich Table is used for summary output.
Verify error handling: try/except wraps run_pipeline, KeyboardInterrupt handled.
  </verify>
  <done>CLI prints Rich table summary after completion. Errors displayed cleanly (no traceback unless --debug). Exit codes correct.</done>
</task>

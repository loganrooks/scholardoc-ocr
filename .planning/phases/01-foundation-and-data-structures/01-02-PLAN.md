...
7. Remove any `sys.exit()` or `raise SystemExit` calls in pipeline.py and processor.py. Library code must raise appropriate exceptions (from scholardoc_ocr.exceptions in a future plan) instead of exiting. For now, replace with `raise RuntimeError(...)` if any are found.
...
Run: `grep -rn 'sys\.exit\|raise SystemExit' src/scholardoc_ocr/pipeline.py src/scholardoc_ocr/processor.py` — must return nothing (library code raises exceptions, never exits).
  </verify>
  <done>pipeline.py has zero Rich imports, uses logging instead, no sys.exit() in library code, stale Levinas naming gone from __init__.py and pipeline.py</done>
</task>
...
6. `grep -rn 'sys\.exit\|raise SystemExit' src/scholardoc_ocr/pipeline.py src/scholardoc_ocr/processor.py` returns nothing
</verification>
